...
PyObject* PyParser::readNumber (PyToken* tok) {
    istringstream tokStr(tok->getLex());
...
PyObject* PyParser::Value(vector<PyCode*>* nestedFunctions) {
    ...
    switch (tok->getType()) {
        ...
        case PYFLOATTOKEN:
            ...
            delete tokStr;
            if (tok->getLex() == "j")
                return new PyComplex(new PyInt(0), new PyFloat(iVal));
            in->putBackToken();
            return new PyFloat(fVal);
            break;
        case PYSTRINGTOKEN:
